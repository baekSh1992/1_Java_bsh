...
public class ArrayExample {
	 ...
	 */
		...
		public void ex2() {
			...
			arr = new int[4]; 
			// heap 영역에 4칸짜리 int 배열을 할당(생성)하고 
			// 할당된 배열의 시작주소(맨 앞)를 arr에 대입
			
			// 배열 길이(크기) : 4
			System.out.println("배열 길이(크기) : " + arr.length );
			// arr.length : arr이 참조하고 있는 배열의 길이(크기)
			
			System.out.println("컴파일러가 정말로 0으로 전부 초기화 해줬을까?");
			
			System.out.println(arr[0]);
			System.out.println(arr[1]);
			System.out.println(arr[2]);
			System.out.println(arr[3]); // 초기화 확인
			
			// stack 영역은 컴파일러가 자동으로 초기화 해주지 않음
			// heap 영역은 컴파일러가 자동으로 초기화 해줌
			int a;
...
//			The local variable a may not have been initialized
			...
			// ** for문을 이용한 배열에 저장된 모든 값 출력 (배열과 단짝) **
...
//			System.out.println( Arrays.toString(heightArr) );
		...
		public void ex6() {	
			...
			String[] menuArr = {"김밥+라면", "백반","순대국","곰탕", "굶기", "삼겹살", "피자","물","오덮","불고기덮밥"};
			
			// 배열 index 범위 내 난수 발생
			int ran = (int)(Math.random() * menuArr.length); // * 배열의 길이만큼
		...
		public void ex7() {	
	      ...
	      // 최저점 : 50
			...
			for(int i =0; i < scoreArr.length; i++) {
				...
				sum += scoreArr[i]; // 점수를 입력 받자마자 바로 sum에 누적
			}
			...
			// for문에 break ponit(클래스 좌측 번호 더블클릭)를 지정하면 증감식이 해석되기 전에 멈춘다. (디버그 모드)
	...
	// 로또 번호 생성기
		public void CreateLottoNumber() {
			...
			for(int i = 0; i < lotto.length; i++) {
				int ran = (int)(Math.random() * 45 + 1); // 1 ~ 45 사이 난수 생성 코드
				
				lotto[i] = ran; // 난수 배열 요소에 대입
		...
		// 깊은 복사 를 위한 배열 선언 및 할당
		int[] copyArr = new int[arr.length];
		
		// 원본 데이터를 모두 복사
		
		// 1) for문을 이용한 방법 (index가 동일 하다는 특징을 이용)
...
//		System.arraycopy( arr, 0, copyArr,0 ,arr.length);
		...
		// 3. 복사할 배열 참조 변수 = Arrays.copy0f(원본배열, 복사할 길이)
		...
		public void ex11(){
			...
			// null 의미
			// - 참조 하는 것(배열,객채)이 없다를 의미 
			
			int[] arr1 = new int[3];
			
			System.out.println( arr1 == null);
			// arr1 = null -> flase == arr1참조 변수가 무언가를 참조하고 있다 
