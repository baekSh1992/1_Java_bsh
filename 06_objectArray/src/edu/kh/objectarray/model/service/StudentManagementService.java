...
public class StudentManagementService {
	...
	public StudentManagementService() {
		...
		stdArr[3] = new Student(3, 8, 6, "장원영", 50, 80, 70);
	}
	
	
	/** 학생 추가 서비스 메서드 (@param = 파라미터)
	 * @param grade
	 * @param classRoom
	 * @param nymber
	 * @param name
	...
	public int addStudent(int grade, int classRoom, int number, String name) {
		...
		for(int i =0; i<stdArr.length; i++) {
			if(stdArr[i] == null) { // 새로운 학생을 추가할 수 있는 자리가 있는 케이스
				idx = 1;
				break;
			}
		}
...
//		return 필드명;
	 ...
	 *   0 : stdArr[idx] 인덱스가 null인 경우(참조 X인 경우)
	 *   1 : 정상적으로 수정이 된 경우
	 */
